...
constexpr Duration STATE_TIMEOUT = 3_min;

// invariant everything is ready!
global_state ready_state_next(global_command cmd,
                              Duration time_since_last_transition) {

  if (cmd == global_command_ABORT || cmd == global_command_EMERGENCY ||
      cmd == global_command_DISCONNECT) {
  ...
  if (cmd == global_command_START_LEVITATION) {
